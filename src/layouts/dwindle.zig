...
pub fn dwindle(monitor: *Monitor) void {
    ...
    const gap_h = if (monitor.smartgaps_enabled and n == 1) 0 else monitor.gap_outer_h;
    const gap_v = if (monitor.smartgaps_enabled and n == 1) 0 else monitor.gap_outer_v;

    const work_x: i32 = monitor.win_x + gap_v;
    const work_y: i32 = monitor.win_y + gap_h;
    const work_w: i32 = monitor.win_w - 2 * gap_v;
    const work_h: i32 = monitor.win_h - 2 * gap_h;
    ...
    while (current) |c| : (current = client_mod.nextTiled(c.next)) {
        ...
        if (can_split) {
            ...
            if (i == 0) {
                if (n != 1) {
                    nw = @intFromFloat(@as(f32, @floatFromInt(work_w)) * monitor.mfact);
                }
                ny = work_y;
            } else if (i == 1) {
                nw = work_w - nw;
